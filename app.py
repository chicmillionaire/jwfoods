...
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{os.path.join(basedir, "jwfoods.db")}'
...
def init_db():
    ...
    with app.app_context():
        ...
        if not DeliveryCoefficients.query.first():
            ...
            db.session.commit()
            print("Default delivery coefficients created")
...
def calculate_delivery():
    ...
    try:
        ...
        if not coeffs:
            return jsonify({'error': 'Delivery coefficients not configured'}), 500
        
        # Calculate delivery cost
        # Formula: base_cost + (distance_coefficient * distance) + (weight_coefficient * weight)
...
def get_coefficients():
    ...
    try:
        coeffs = DeliveryCoefficients.query.first()
        if not coeffs:
            return jsonify({'error': 'No coefficients found'}), 404
